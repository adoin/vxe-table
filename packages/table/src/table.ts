...
export default defineComponent({
  ...
  setup (props, context) {
        ...
        if (!allRemoteSort && orderColumns.length) {
          if (treeConfig && transform) {
            ...
            if (allSortMethod) {
              ...
              tableTree = XEUtils.orderBy(tableTree, orderColumns.map(({
                column,
                order
            ...
            if (allSortMethod) {
              ...
              tableData = XEUtils.orderBy(tableData, orderColumns.map(({
                column,
                order
              }) => [getOrderField(column), order]))
            }
    ...
    const handleBaseTreeExpand = (rows: any[], expanded: boolean) => {
      ...
      let validRows = toggleMethod ? rows.filter((row: any) => toggleMethod({
        $table: $xetable,
        expanded,
    ...
    tablePrivateMethods = {
      ...
      updateScrollXSpace () {
        ...
        if (tableBodyElem) {
          ...
          const bodyElem = isFunction(tableBodyElem.querySelector) ? (tableBodyElem.querySelector('.vxe-table--body') as HTMLTableElement) : null
